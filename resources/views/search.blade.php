                ...
                <div class="row">
                  <div class="col-sm-3">
                    ...
                    <div class="movie-series-img">
                      @if(!is_null($actor->image))
                        <img src="{{asset('images/actors/'.$actor->image)}}" class="img-responsive actor_image" alt="genre-image">
                      @else
                        <img src="{{asset('images/default-thumbnail.jpg')}}" class="img-responsive actor_image" alt="genre-image">
                  ...
                  <div class="col-sm-7 pad-0">
                    <h5 class="movie-series-heading movie-series-name">
                      ...
                      <p>
                       {{$home_translations->where('key', 'Actor DOB')->first->value['value']}}- {{$actor->DOB}} </p>
                    <p>
                       {{$home_translations->where('key', 'Actor Place Of Birth')->first->value['value']}}- {{$actor->place_of_birth}} </p>
                  ...
                  <div class="col-sm-7 pad-0">
                    <h5 class="movie-series-heading movie-series-name">
                      ...
                      @elseif($item->type == 'S')
                        ...
                        @else
                          <a href="{{url('show/guest/detail', $item->id)}}">{{$item->tvseries->title}}</a>
                        @endif
                      @endif
                    </h5>
                    <ul class="movie-series-des-list">
                      @if($item->type == 'M')
                        <li>{{ $home_translations->where('key', 'TMDB Rating')->first->value['value']}} {{$item->rating}}</li>
                      @endif
                      @if($item->type == 'S')
                        <li>{{ $home_translations->where('key', 'TMDB Rating')->first->value['value']}} {{$item->tvseries->rating}}</li>
                      @endif
                      <li>
                        @if($item->type == 'M')
                      ...
                      @if($item->type == 'M')
                       @if($item->video_link['iframeurl'] != null)
                        ...
                        @if($item->trailer_url != null || $item->trailer_url != '')
                          ...
                          <a href="{{ route('watchTrailer',$item->id) }}" class="iframe btn btn-default">{{$popover_translations->where('key', 'watch trailer')->first->value['value']}}</a>
                        @endif
                      ...
                      @else
                        @if(isset($item->episodes[0]))
                           @if($item->episodes[0]->video_link->iframeurl !="")
                            ...
                            <a href="#" onclick="playoniframe('{{ $item->episodes[0]->video_link->iframeurl }}','{{ $item->id }}','tv')" class="btn btn-play"><span class="play-btn-icon"><i class="fa fa-play"></i></span> <span class="play-text">{{$popover_translations->where('key', 'play')->first->value['value']}}</span>
                             </a>
                    ...
                    @else
                     <div class="des-btn-block des-in-list">
                        @if($item->trailer_url != null || $item->trailer_url != '')
                          ...
                          <a href="{{ route('guestwatchtrailer',$item->id) }}" class="iframe btn btn-default">{{$popover_translations->where('key', 'watch trailer')->first->value['value']}}</a>
                        @endif
                      </div>
                    @endif
                  </div>
